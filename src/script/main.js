...
function Gallery(photos, cursor = 3) {
  function next(options) {
    cursor = cursor + 1 > photos.length - 1 ? 0 : cursor + 1;
  ...
  function render({ url, description }) {
    $Testi[0].style.backgroundImage = `url(${url})`;
    $('.header__copyright')[0].innerText = description;
  }
  ...
  const findNextState = (function() {
    ...
    return () => {
      keys.forEach(k => {
        ...
        State[k].cur = Math.max(
          Math.min((Math.random() > 0.5 ? 1 : -1) * step, max),
          min
        );
      });
      apply(State);
...
const { findNextState: nextBackgroundAnimation } = BackgroundAnimation(
  $Testi[0],
  1500
);
...
// UI interactions
$Testi[0].addEventListener('click', nextSlide, false);
